...
bool UsbReader::process_usb_msg(msg m) {
  ...
  this->l->log(Logger::LogLevel::WARNING, std::to_string(m.size()));
  ...
  if (m.size() == 1) {
    ...
    if (m[0] == "d") {
      ...
      usb_commands.push_back(std::make_shared<LuminaireConfig>(
      ...
      std::cout << "Reference message " << std::endl;
      std::string lum = m[1];
      std::string ref = m[2];
      usb_commands.push_back(std::make_shared<LuminaireConfig>(
          LuminaireConfig(CONFIG::SET, TYPE::REF, ref, lum)));
    } else if (m[0] == "g" && m[1] == "r") {
      std::string lum = m[2];
      usb_commands.push_back(std::make_shared<LuminaireConfig>(
          LuminaireConfig(CONFIG::GET, TYPE::REF, "", lum)));
    } else if (m[0] == "g" && m[1] == "l") {
      std::string lum = m[2];
      usb_commands.push_back(std::make_shared<LuminaireConfig>(
      ...
      usb_commands.push_back(
          std::make_shared<StreamCmd>(StreamCmd(STREAM::START, lum, state)));
    } else if (m[0] == "p") {
      std::string lum = m[1];
      std::string state = m[2];
      usb_commands.push_back(
          std::make_shared<StreamCmd>(StreamCmd(STREAM::STOP, lum, state)));
    } else if (m[0] == "g" && m[1] == "e") {
      std::string lum = m[2];
      usb_commands.push_back(std::make_shared<MonitorCmd>(
          MonitorCmd(SYSTEM_MONITOR::AVERAGE_ENERGY, lum)));
    } else if (m[0] == "g" && m[1] == "v") {
      std::string lum = m[2];
      usb_commands.push_back(std::make_shared<MonitorCmd>(
          MonitorCmd(SYSTEM_MONITOR::VISIBILITY_ERROR, lum)));
    } else if (m[0] == "g" && m[1] == "f") {
      std::string lum = m[2];
      usb_commands.push_back(std::make_shared<MonitorCmd>(
          MonitorCmd(SYSTEM_MONITOR::AVERAGE_FLICKER, lum)));
    } else {
